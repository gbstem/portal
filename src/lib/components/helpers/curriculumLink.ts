export function generateCurriculumLink(className: string): string {
    if (className.includes("Mathematics")) {
        ...
        const adjustedCase = adjustedName.slice(0, -1).toLowerCase() + adjustedName.slice(-1).toUpperCase();
        ...
        const adjustedName = className.replace("Environmental Science", "environmental");
        const adjustedCase = adjustedName.slice(0, -1).toLowerCase() + adjustedName.slice(-1).toUpperCase();
